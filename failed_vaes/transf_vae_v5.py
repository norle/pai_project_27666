...
class TransformerVAE(nn.Module):
    def __init__(self, vocab_size, d_model=512, nhead=8, num_layers=6, dim_feedforward=2048, dropout=0.1, max_len=5000, latent_dim=16):
        ...
        self.fc_encoder_to_latent = nn.Sequential(
            nn.Linear(d_model, latent_dim * 2),
            nn.ReLU(),
            nn.Linear(latent_dim * 2, latent_dim)
        )
        
        self.fc_latent_to_decoder = nn.Sequential(
    ...
    def encode(self, src, src_mask=None, src_padding_mask=None):
        src = self.src_embedding(src) * (self.src_embedding.embedding_dim ** 0.5)
        src = src.transpose(0, 1)
        src = self.positional_encoding(src)
        memory = self.encoder(src, src_mask, src_padding_mask)
        memory = memory.mean(dim=0)  # Aggregate over sequence length
    ...
    def decode(self, z, trg, trg_mask=None, trg_padding_mask=None, memory_key_padding_mask=None):
        z = self.fc_latent_to_decoder(z).unsqueeze(0)  # Add sequence dimension
        trg = self.trg_embedding(trg) * (self.trg_embedding.embedding_dim ** 0.5)
        trg = trg.transpose(0, 1)
        trg = self.positional_encoding(trg)
        output = self.decoder(trg, z, trg_mask, None, trg_padding_mask, memory_key_padding_mask)
...
if model_path != None:
    ...
    model = TransformerVAE(vocab_size, model_dim, num_heads, num_layers, dim_feedforward=1024, dropout=0.1, max_len=seq_length, latent_dim=latent_dim).to(device)
